...
class Manager:
    def __init__(self, inkscape_id):
        ...
        self.inkscape = self.disp.create_resource_object('window', self.id)
...
def is_inkscape(window):
    return (window.get_wm_class() and
            'inkscape' in window.get_wm_class()[0])
...
def main():
    ...
    for window in root.query_tree().children:
        if is_inkscape(window):
            print('Found existing inkscape window')
    ...
    while True:
        ...
        if evt.type == X.CreateNotify:
            ...
            try:
                if is_inkscape(window):
                    print('Found new inkscape window')
                    listen = threading.Thread(target=create, args=[window.id])
                    listen.start()
